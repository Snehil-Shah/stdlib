...
tape( 'an instance throws an error if provided a `bufferSize` which is less than the number of data elements', function test( t ) {
	...
	function badValue( value ) {
		return function badValue() {
			var table = ctor( [ [ 1, 2 ], [ 3, 4 ], [ 5, 6 ], [ 7, 8 ] ] );
			table.bufferSize = value;
		};
	}
});
...
tape( 'an instance supports setting and getting the property value', function test( t ) {
	...
	table.render = noop;
	t.strictEqual( table.bufferSize, 10, 'returns expected value' );

	table.bufferSize = 20;
	t.strictEqual( table.bufferSize, 20, 'returns expected value' );

	table.bufferSize = 30;
	t.strictEqual( table.bufferSize, 30, 'returns expected value' );

	table.bufferSize = null;
	t.strictEqual( table.bufferSize, FLOAT64_MAX, 'returns expected value' );
