...
function wrappedColumnIndices( ctx ) {
	...
	for ( i = 0; i < ncols; i++ ) {
		...
		if ( w > ctx.maxWidth ) {
			...
			return new Error( format( 'invalid operation. The resolved column width for column %d exceeds the maximum allowed table width. Consider either increasing `maxWidth` or decreasing `maxColumnWidth`. Maximum width: %d. Maximum cell width: %d. Column width: %d.', ctx.maxWidth, mw, ctx.columnWidths[ i ] ) );
		}
		width += w;
...
function render() {
	...
	idx = wrappedColumnIndices({
		...
		'maxWidths': this._maxColumnWidth,
