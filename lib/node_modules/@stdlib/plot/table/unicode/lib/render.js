...
* @param {Array<NonNegativeInteger|null>} ctx.fixedWidths - fixed column widths (in units of grapheme clusters)
* @param {NonNegativeIntegerArray} ctx.computedWidths - column widths (in units of grapheme clusters)
* @param {NonNegativeIntegerArray} ctx.maxColumnWidths - maximum column widths (in units of grapheme clusters)
...
function wrappedColumnIndices( ctx ) {
	...
	for ( i = 0; i < ncols; i++ ) {
		...
		if ( i < ncols-1 ) {
			width += ctx.columnSeparator.ngraphemes;
		}
	}
...
function renderLine( start, end, ch, ctx ) {
	...
	for ( i = start; i < end; i++ ) {
		...
		if ( i > start && ctx.columnSeparator.ngraphemes > 0 ) {
			out += ctx.jointM;
			if ( ctx.mode === 'interpolate' ) {
				hidx += 1;
...
* @param {Object} ctx - context object
* @param {Array<NonNegativeInteger|null>} ctx.fixedWidths - fixed column widths (in units of grapheme clusters)
* @param {NonNegativeInteger} ctx.computedWidths - column widths (in units of grapheme clusters)
* @param {NonNegativeIntegerArray} ctx.maxColumnWidths - maximum column widths (in units of grapheme clusters)
* @param {NonNegativeInteger} ctx.marginL - left table margin
* @param {NonNegativeInteger} ctx.marginR - right table margin
...
function renderRow( row, start, end, data, ctx, vidx ) {
	...
	// Render left border:
	if ( ctx.borderL.ngraphemes > 0 ) {
		out += ctx.borderL.graphemes[ vidx%ctx.borderL.ngraphemes ];
	}
	for ( i = start; i < end; i++ ) {
		// Render column separator:
		if ( i > start && ctx.columnSeparator.ngraphemes > 0 ) {
			out += ctx.columnSeparator.graphemes[ vidx%ctx.columnSeparator.ngraphemes ]; // eslint-disable-line max-len
		}
		...
		// Resolve cell data:
		v = data.get( row, i );
		len = lpad + v.ngraphemes + rpad; // number of grapheme clusters
	...
	// Render right border:
	if ( ctx.borderR.ngraphemes > 0 ) {
		out += ctx.borderR.graphemes[ vidx%ctx.borderR.ngraphemes ];
	}
	// Render right margin:
	out += renderWhitespace( ctx.marginR );
...
function render() {
	...
	headers = ( this._headers ) ? array2matrix( this._headers, [ 1, this._headers.length ] ) : null; // eslint-disable-line max-len
	...
	idx = wrappedColumnIndices({
		...
		'borderL': this._borders[ 3 ].ngraphemes,
		'borderR': this._borders[ 1 ].ngraphemes,
	...
	for ( j = 0; j < idx.length-1; j++ ) {
		...
		// Render top border:
		if ( this._borders[ 0 ].ngraphemes > 0 ) {
		...
		if ( this._headers && this._headerSeparator.ngraphemes !== 0 ) {
		...
		if ( this._data ) {
			for ( i = 0; i < nrows; i++ ) {
				...
				if ( this._rowSeparator.ngraphemes > 0 && i < nrows-1 ) {
					lctx.jointL = this._joints[ 4 ].value;
					lctx.jointM = this._joints[ 0 ].value;
					lctx.jointR = this._joints[ 2 ].value;
		...
		if ( this._borders[ 2 ].ngraphemes > 0 ) {
