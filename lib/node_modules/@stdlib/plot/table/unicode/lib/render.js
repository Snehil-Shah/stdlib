...
function wrappedColumnIndices( ctx ) {
	...
	for ( i = 0; i < ncols; i++ ) {
		...
		if ( i < ncols-1 ) {
			width += ctx.columnSeparator.columns;
		}
	}
...
function renderLine( start, end, ch, ctx ) {
	...
	for ( i = start; i < end; i++ ) {
		...
		if ( i > start && ctx.columnSeparator.columns > 0 ) {
			out += ctx.jointM;
			if ( ctx.mode === 'interpolate' ) {
				hidx += 1;
...
function renderRow( row, start, end, data, ctx, vidx ) {
	...
	for ( i = start; i < end; i++ ) {
		...
		// Resolve cell data:
		v = data.get( row, i );
		len = lpad + v.columns + rpad; // number of character columns
		...
		if ( w > mw ) {
			...
			w = mw;
			tmp = v.graphemes.slice( 0, mw-lpad-rpad ); // FIXME: be emoji aware and don't cut off graphemes mid-grapheme
			d = tmp.join( '' );
		} else {
			d = v.value;
		}
...
function render() {
	...
	idx = wrappedColumnIndices({
		...
		'borderL': this._borders[ 3 ].columns,
		'borderR': this._borders[ 1 ].columns,
	...
	for ( j = 0; j < idx.length-1; j++ ) {
		...
		if ( this._headers && this._headerSeparator.columns !== 0 ) {
		...
		if ( this._data ) {
			for ( i = 0; i < nrows; i++ ) {
				...
				if ( this._rowSeparator.columns > 0 && i < nrows-1 ) {
					lctx.jointL = this._joints[ 4 ].value;
					lctx.jointM = this._joints[ 0 ].value;
					lctx.jointR = this._joints[ 2 ].value;
