...
function set( i, j, value ) {
	...
	// Assume that, if a user is setting a new value, that means the data is different... (note: this provides an escape hatch for the same instance of a mutable class to be provided and have that instance trigger a 'change' event when its internal data/state has changed without the **reference** to that class instance having changed; meaning, two values may strictly compare as equal, but are allowed to render differently due to internal state changes)
	fmt = this._format[ j%this._format.length ];
	v = normalizeData( [ value ], [ 1, 1 ], [ fmt ] );
