...
/**
* Sets the bottom-left table corner character.
*
* @private
...
function set( value ) {
	var w;
	if ( !isString( value ) ) {
		throw new TypeError( format( 'invalid assignment. `%s` must be a string which contains one and only one character. Value: `%s`.', 'cornerBottomLeft', value ) );
	}
	w = wcswidth( value );
	if ( w !== 1 ) {
		throw new TypeError( format( 'invalid assignment. `%s` must be a string which contains one and only one character. Value: `%s`.', 'cornerBottomLeft', value ) );
