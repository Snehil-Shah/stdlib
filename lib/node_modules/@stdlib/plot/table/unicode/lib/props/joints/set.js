...
/**
* Sets the joint grapheme clusters (i.e., visual characters) in the order `[middle, top, right, bottom, left ]`.
*
* @private
...
function set( values ) {
	...
	for ( i = 0; i < NUM_JOINTS; i++ ) {
		v = values[ i ];
		if ( !isString( v ) ) {
			throw new TypeError( format( 'invalid assignment. `%s` must be an array of strings in which each array element is a single grapheme cluster. Value: `["%s"]`.', 'joints', copy( values ).join( '", "' ) ) );
		}
