...
function set( values ) {
	...
	for ( i = 0; i < NUM_BORDERS; i++ ) {
		v = values[ i ];
		if ( !isString( v ) ) {
			throw new TypeError( format( 'invalid assignment. `%s` must be an array of strings in which each array element is a string containing one and only one character. Value: `["%s"]`.', 'borders', copy( values ).join( '", "' ) ) );
		}
		w = wcswidth( v );
		if ( w !== 1 ) {
			throw new TypeError( format( 'invalid assignment. `%s` must be an array of strings in which each array element is a string containing one and only one character. Value: `["%s"]`.', 'borders', copy( values ).join( '", "' ) ) );
